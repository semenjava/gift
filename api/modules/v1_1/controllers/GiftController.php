...
class GiftController extends ActiveController {
    ...
    public function actionGet($id) {
        ...
        if ($gift) {
			...
			if(!empty($gift['dates']['custom_holiday'])) {
				$gift['dates']['custom_holiday'] = StringHelper::decodeEmoji($gift['dates']['custom_holiday']);
			}
            return $this->responseSuccess($gift);
        }
        return $this->responseFail('There is no such by gift '.$id);
    }
    ...
    public function actionGetList($id_dates) {
            ...
            foreach ($gift as $key => $value) {
                ...
                if(!empty($value['dates'])) {
                    if(!empty($value['dates']['custom_holiday'])) {
                        $gift[$key]['dates']['custom_holiday'] = StringHelper::decodeEmoji($value['dates']['custom_holiday']);
                    }
                }
            }
        ...
        return $this->responseFail('There is no such by gift id '.$id);
    }
