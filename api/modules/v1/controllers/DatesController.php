...
class DatesController extends ActiveController {
    ...
    public function actionGet($id) {
        ...
        if ($dates) {
            if(!empty($dates['custom_holiday'])) {
                $dates['describe'] = StringHelper::decodeEmoji($dates['custom_holiday']);
            }
        ...
        return $this->responseFail($dates);
    }
        ...
        if ($dates) {
            foreach ($dates as $key => $value) {
                if(!empty($value['custom_holiday'])) {
                    $dates[$key]['describe'] = StringHelper::decodeEmoji($value['custom_holiday']);
                }
    ...
    public function actionCreate() {
		...
		if(!empty($post['Dates']['id_date_desc'])) {
			$post['Dates']['id_holiday'] = $post['Dates']['id_date_desc'];
		}

		if(!empty($post['Dates']['describe'])) {
			$post['Dates']['custom_holiday'] = $post['Dates']['describe'];
		}
		...
		if ($dates->load($post) && $gift->load($post)) {
			if ($dates->validate() && $gift->validate([
				...
				return $this->responseFail($errors);
			}
		}
    }
